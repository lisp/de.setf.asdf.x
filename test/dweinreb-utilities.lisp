...
(defclass test ()
 ((system-name :initarg :system-name :reader test-system-name)
  ...
  (equal :initarg :equal :reader test-equal)
...
(defmacro define-test (test-name system-name 
                       ...
                       already-compiled expected equal)
 `(progn
...
(defmethod operation-done-p ((o compile-op) (c test-file))
 (declare (ignorable o))
 (or
  (member (component-name c) (test-already-compiled *test*) 
          :test #'string=)
  (call-next-method)))

(defmethod operation-done-p ((o operation) (c test-file))
  (find (cons (ecase (type-of o) (load-op :loaded) (compile-op :compiled))
              (component-name c))
        *steps*
        :test #'equal))

(defmethod perform ((o compile-op) (c test-file))
 (declare (ignorable o))
 (push (cons :compiled (component-name c)) *steps*))

(defmethod perform ((o load-op) (c test-file))
 (declare (ignorable o))
   ...
   (flet ((fail (format-string &rest format-args)
     ...
     (let ((system-name (test-system-name *test*))
        ...
        (dolist (operation operations)
          ...
          (cond ((subtypep operation 'operation)
                 (operate operation system-name))
                ((fboundp operation)
                 (funcall operation system-name))
                (t
                 (error "Invalid test operation: ~s." operation)))))
     (setq *steps* (nreverse *steps*))
     (cond ((test-equal *test*)
            (unless (equal (test-equal *test*) *steps*)
              (fail "Result does not match expectation:~% test:   ~s~% result: ~s"
                    (test-equal *test*) *steps*)))
           (t
